...
export const moneyPages: Page[] = [
  {
    ...
    seoTitle: "Preços IPTV Portugal 2026: Planos desde 15€ | Tabela",
    ...
    summary: `Um plano de IPTV custa ${formatEuro(lowestPrice)} por 1 mês com 1 dispositivo e ${formatEuro(priceFor(4, 12))} por 12 meses com 4 dispositivos (cerca de ${formatEuro(lowestMonthly)} por mês). Todos os planos incluem ${facts.channels}, ${facts.vod}, guia TV e apoio ${offer.support}, sem fidelização. Cada duração inclui um número diferente de dispositivos.`,
    updated,
    blocks: [
      { t: "pricing" },
      ...
      {
        t: "p",
        text: `O preço muda com a **duração** e com o **número de ecrãs a ver ao mesmo tempo**. Cada plano já inclui de 1 a 4 dispositivos; só se paga um adicional ao ultrapassar esse número. O conteúdo é igual em todos os planos.`,
      },
      { t: "facts" },
      { t: "h2", text: "Tabela completa de preços" },
      {
        t: "table",
        caption: "Preço base em euros e dispositivos incluídos em cada plano",
        head: ["Duração", "Dispositivos incluídos", "Preço base", "Dispositivo extra"],
        rows: offer.months.map((m) => [
          m === 1 ? "1 mês" : `${m} meses`,
          String(offer.plans[m].includedDevices),
          formatEuro(offer.plans[m].price),
          formatEuro(offer.plans[m].extraDevicePrice),
        ]),
      },
      { t: "h2", text: "Que plano faz mais sentido para ti?" },
      {
        ...
        items: [
          ...
          `**12 meses** — cerca de ${formatEuro(lowestMonthly)} por mês com 4 dispositivos incluídos.`,
        ],
      },
      {
        t: "callout",
